...
## Design Notes
## Problem framing
Accepted system understanding disappears quickly if it lives only in README edits, PR descriptions, or chat summaries. Pi Loom therefore needs a documentation layer that captures high-level explanatory truth after work is complete without confusing docs with plans, tickets, or critique.

## Desired behavior
Documentation should behave as a curated explanatory corpus for accepted architecture, workflows, concepts, and operations. The layer must support both direct known-state writes and bounded fresh-context maintainer passes, preserve revision history and provenance, and remain clearly distinct from raw repository files or execution records.

## Scope
This spec covers documentation records, direct write versus packetized update behavior, upstream file ingestion and provenance, revision history, multi-repository output/path truth, and the post-completion boundary between docs and neighboring layers. Governance topics such as topic ownership and drift classification are intentionally adjacent to, not redefined by, the existing curated documentation governance spec.

## Non-goals
This spec does not replace the separate documentation governance contract, does not turn docs into API reference generation, and does not let pre-completion execution notes masquerade as accepted documentation truth.

## Dependencies and adjacent specs
Docs depend on constitution, research, initiatives, specs, plans, tickets, and critique for context, on storage for canonical persistence, and on the existing curated documentation governance spec for topic ownership, lifecycle, and audit semantics.

## Risks and edge cases
The biggest risks are updating docs before reality is accepted, mixing live execution chatter into explanatory docs, or losing provenance to upstream work or repository source files. Another risk is silently mutating raw repo docs without durable metadata linking them back to canonical documentation records.

## Verification expectations
A conforming documentation record can be read later and still explain what topic it covers, why it matters, what upstream accepted reality it reflects, when it was verified, and how it relates to existing repository documentation or linked outputs.

## Provenance
Derived from README.md, docs/README.md, documentation layer guidance, docs tool contracts, and the existing curated documentation governance spec boundary.

## Open questions
Governed topic taxonomy may evolve, but the documentation layer must remain the post-completion explanatory memory surface with durable provenance and revision history.
