...
## Design Notes
The runtime now prefers a minimal session-runtime adapter over shelling back into the CLI. It dynamically resolves the current harness SDK package, creates an in-memory session, binds extensions in headless mode, activates the full tool surface needed for Ralph and Chief loops, and forwards runtime-only parent session metadata through the in-process Chief scheduler. Chief's background execution is event-loop scheduling inside the parent process rather than detached script daemons.
