...
describe("research store", () => {
  ...
  it("rebuilds research artifacts from canonical artifact entities without leaving stale ids", async () => {
    ...
    const { storage, identity } = await openWorkspaceStorage(workspace);
    const artifactEntity = (await storage.listEntities(identity.space.id, "artifact")).find(
      (entity) => entity.displayId === "research:evaluate-cache-invalidation:artifact:note:artifact-001",
    );
