...
export interface HarnessRuntimeUsage {
  measured: boolean;
  input: number;
  output: number;
  cacheRead: number;
...
export interface HarnessLaunchStateEvent {
  type: "launch_state";
  state: "queued" | "running";
  at: string;
}

export interface HarnessToolExecutionEvent {
  type: "tool_execution";
  phase: "start" | "end";
  toolName: string;
...
export interface HarnessAssistantMessageEvent {
  type: "assistant_message";
  text: string;
  at: string;
}

export type HarnessRuntimeEvent = HarnessLaunchStateEvent | HarnessToolExecutionEvent | HarnessAssistantMessageEvent;
...
export const PI_HARNESS_COMMAND_ENV = "PI_HARNESS_COMMAND";
export const PI_HARNESS_PACKAGE_ROOT_ENV = "PI_HARNESS_PACKAGE_ROOT";
...
function emptyUsage(): HarnessRuntimeUsage {
  return { measured: false, input: 0, output: 0, cacheRead: 0, cacheWrite: 0, totalTokens: 0 };
}

function mergeUsage(current: HarnessRuntimeUsage, usage: SessionMessage["usage"]): HarnessRuntimeUsage {
  if (!usage) {
    return current;
  }
...
function resolveHarnessPackageInfo(deps: PiSpawnDeps = {}): HarnessPackageInfo | null {
  ...
  const overriddenRoot = trimToUndefined(env[PI_HARNESS_PACKAGE_ROOT_ENV]);
  if (overriddenRoot) {
...
export function getHarnessSpawnCommand(args: string[], deps: PiSpawnDeps = {}): PiSpawnCommand {
  const env = deps.env ?? process.env;
  const overriddenCommand = trimToUndefined(env[PI_HARNESS_COMMAND_ENV]);
...
function appendEvent(
  ...
  onEvent?: (event: HarnessRuntimeEvent) => void | Promise<void>,
): void {
...
export function resolveExtensionRoot(startDir = path.dirname(fileURLToPath(import.meta.url))): string {
...
export async function runHarnessLaunch(
  cwd: string,
  prompt: string,
  ...
  const resolvedExtensionRoot = extensionRoot ?? resolveExtensionRoot();
  const sessionDir = fs.mkdtempSync(path.join(tmpdir(), "pi-loom-harness-"));
  const command = getHarnessSpawnCommand(
    ["-e", resolvedExtensionRoot, "--mode=json", "-p", prompt, "--session-dir", sessionDir],
    { env: runtimeEnv },
  );
  ...
  const emitEvent = (event: HarnessRuntimeEvent) => appendEvent(state, event, onEvent);
  ...
  const status: HarnessExecutionResult["status"] = failedByAbort
    ? "failed"
