...
- use the ticket layer truthfully: plans wrap those tickets in broader execution context, and each linked ticket stands alone as a complete unit of work whether it already existed or was created alongside the plan
- when the execution slice is already clear and you can still make every ticket fully detailed, prefer the cohesive `plan_write` path that materializes linked tickets in the same write; when ticket authorship needs more room, create or update the plan first and materialize or link tickets later instead of generating thin tickets
- treat constitutional memory as the durable project-policy layer, research as the evidence layer, initiatives as strategic context, specs as declarative specifications of intended behavior, tickets as the live execution ledger and the complete definition of each execution unit, critique as the adversarial review layer, and docs as the post-completion explanatory layer
...
- keep linked ticket integration Loom-native: plans can summarize ticket roles and current status, but tickets remain the source of live execution truth and carry their own acceptance criteria, dependencies, and execution notes. `linkedTickets` is active membership; `contextRefs.ticketIds` is only loose packet context
