...
- `plan_*` tools for list/read/write/packet/ticket-link/overview workflows
...
- optional integrated ticket materialization through `plan_write`, so a caller can create or update a plan and create/link a detailed execution slice in one cohesive workflow when the ticket set is already clear
...
- rendered plan output that is self-contained and ExecPlan-shaped for novice readers, with living-document sections such as `Progress`, `Milestones`, `Validation and Acceptance`, `Idempotence and Recovery`, `Interfaces and Dependencies`, and `Revision Notes`

## Planning semantics

`pi-loom` keeps planning distinct from specs and from tickets.

- plans are more bounded than initiatives and more strategic than ticket-by-ticket execution history
- specs remain standalone declarative behavior contracts; plans do not rewrite that contract, they sequence how the current repository will satisfy it
...
- `plan_write` is the primary cohesive authoring surface when the execution slice is already clear enough to create fully detailed tickets in the same call; if ticket writing needs more room, create or update the plan first and materialize or link tickets later through the ticket layer and `plan_ticket_link`
- the rendered plan is a self-contained novice-facing execution narrative with sequencing, rationale, milestones, recovery guidance, interfaces, revision history, and validation intent, while linked tickets carry the live work state and the detailed per-unit work definitions
...
- `linkedTickets` means active plan membership, while `contextRefs.ticketIds` are only loose packet context; do not treat them as interchangeable
