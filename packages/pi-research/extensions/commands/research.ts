...
export async function handleResearchCommand(args: string, ctx: ExtensionCommandContext): Promise<string> {
  ...
  switch (subcommand) {
    ...
    case "create": {
      ...
      return renderResearchDetail(store.createResearch({ title }));
    }
    case "list": {
    ...
    case "show": {
      ...
      return renderResearchDetail(store.readResearch(ref));
    }
    case "update": {
      const { ref, updates } = parseUpdateArgs(rest);
      return renderResearchDetail(store.updateResearch(ref, updates));
    }
    case "hypothesis": {
      const { ref, input } = parseHypothesisArgs(rest.join(" "));
      return renderResearchDetail(store.recordHypothesis(ref, input));
    }
    case "artifact": {
      const { ref, input } = parseArtifactArgs(rest.join(" "));
      return renderResearchDetail(store.recordArtifact(ref, input));
    }
    case "dashboard": {
      const ref = rest[0];
      if (!ref) throw new Error("Usage: /research dashboard <research>");
      return renderResearchDashboard(store.readResearch(ref).dashboard);
    }
    case "map": {
      const ref = rest[0];
      if (!ref) throw new Error("Usage: /research map <research>");
      return renderResearchMap(store.readResearch(ref).map);
    }
