...
- a polling daemon that watches durable state between raw Ralph loop iterations

A manager is itself a Ralph loop. Workers are internal ticket-bound Ralph loops running in dedicated worktrees. The daemon multiplexes those loops by polling durable state and only re-entering manager reasoning when no loops are running and the manager actually needs to think again.
...
- `manager_start` — create a manager from a spec, initiative, plan, ticket set, or free-text objective and start its background daemon
- `manager_wait` — block until the manager has something to say or finishes
- `manager_steer` — answer escalations, provide guidance, record review decisions, or change the target ref, then let the background daemon continue
...
3. When ticket work should exist, the manager reconciles ticket-bound workers and starts their Ralph loops inside dedicated git worktrees.
4. The daemon polls durable state while manager or worker loops are running.
5. The daemon does not re-enter the manager loop while any loops are still running.
...
These tools are only registered when `PI_CHIEF_INTERNAL_MANAGER=1` so normal sessions stay clean.
