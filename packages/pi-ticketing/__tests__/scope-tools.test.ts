...
import { closeAllWorkspaceStorage } from "@pi-loom/pi-storage/storage/workspace.js";
...
describe("scope tools", () => {
  ...
  it("reads, selects, revokes, unenrolls, and reenrolls scope through headless tools", async () => {
    ...
    try {
      ...
      const firstRepositoryId = (initial.details as { scope: { enrolledRepositories: Array<{ repository: { id: string } }> } }).scope
        .enrolledRepositories[0]?.repository.id;
      ...
      expect(unenrolled.details).toMatchObject({
        action: "unenroll",
        scope: {
          enrolledRepositories: [expect.not.objectContaining({ repository: expect.objectContaining({ id: firstRepositoryId }) })],
