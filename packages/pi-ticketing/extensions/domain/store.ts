...
export class TicketStore {
  ...
  private nextTicketId(records: TicketReadResult[]): string {
    return nextNumericId(records.map((record) => record.summary.id), "t");
  }
  ...
  async updateTicketAsync(ref: string, updates: UpdateTicketInput): Promise<TicketReadResult> {
    ...
    this.validateRelationships(record.frontmatter.id, record.frontmatter.deps, record.frontmatter.parent, this.canonicalSummaries(existingRecords));
    if (record.frontmatter.status !== "closed") {
      this.assertTransitionAllowed(record.frontmatter.id, record.frontmatter.deps, record.frontmatter.status, this.canonicalSummaries(existingRecords));
    }
  ...
  async startTicketAsync(ref: string): Promise<TicketReadResult> {
    const current = await this.readTicketAsync(ref);
    this.assertTransitionAllowed(current.summary.id, current.ticket.frontmatter.deps, "in_progress", this.canonicalSummaries(await this.canonicalRecords()));
    const timestamp = currentTimestamp();
  ...
  async addDependencyAsync(ref: string, depRef: string): Promise<TicketReadResult> {
    ...
    this.validateRelationships(current.summary.id, [...current.ticket.frontmatter.deps, depId], current.ticket.frontmatter.parent, this.canonicalSummaries(await this.canonicalRecords()));
    const timestamp = currentTimestamp();
