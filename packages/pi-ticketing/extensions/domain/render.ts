...
export function renderTicketDetail(result: TicketReadResult): string {
  ...
  return [
    ...
    `Spec change: ${ticket.frontmatter["spec-change"] ?? "none"}`,
    `Spec capabilities: ${renderList(ticket.frontmatter["spec-capabilities"])}`,
    `Spec requirements: ${renderList(ticket.frontmatter["spec-requirements"])}`,
