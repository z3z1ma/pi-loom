...
export function parseMarkdownArtifact(
  ...
  if (lines[0] !== "---") {
    throw new Error(`Markdown artifact ${filePath} is missing frontmatter`);
  }
  const endIndex = lines.indexOf("---", 1);
  if (endIndex === -1) {
    throw new Error(`Markdown artifact ${filePath} has unterminated frontmatter`);
  }
  ...
  for (const line of lines.slice(1, endIndex)) {
    ...
    if (line.startsWith("  - ")) {
      if (!currentArrayKey) {
        throw new Error(`Invalid array entry in ${filePath}`);
      }
