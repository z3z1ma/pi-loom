...
function hasObjectSchemaProperties(
  parameters: ToolDefinition["parameters"] | undefined,
): parameters is NonNullable<ToolDefinition["parameters"]> & { properties: Record<string, unknown> } {
  return (
    !!parameters &&
...
describe("manager tools", () => {
  ...
  it("exposes gitBaseRef on the public manager tool surface", () => {
    const mockPi = createMockPi();
    registerManagerTools(mockPi as unknown as ExtensionAPI);

    const managerStart = mockPi.tools.get("manager_start");
    const managerSteer = mockPi.tools.get("manager_steer");
    expect(managerStart && managerSteer).toBeTruthy();

    const managerStartParameters = managerStart?.parameters;
    expect(managerStartParameters).toBeDefined();
    expect(hasObjectSchemaProperties(managerStartParameters)).toBe(true);
    if (!hasObjectSchemaProperties(managerStartParameters)) {
      throw new Error("manager_start parameters should be an object schema with properties");
    }
    expect(managerStartParameters).toMatchObject({
      properties: expect.objectContaining({
        gitBaseRef: expect.objectContaining({ type: "string", optional: true }),
      }),
    });
    expect(Object.keys(managerStartParameters.properties)).toEqual(
      expect.arrayContaining(["gitBaseRef", "linkedRefs", "objective", "summary", "timeoutMs", "title", "wait"]),
    );
    expect(managerStart?.description).toContain("git base ref");
    expect(managerStart?.description?.toLowerCase()).not.toContain("target ref");

    const managerSteerParameters = managerSteer?.parameters;
    expect(managerSteerParameters).toBeDefined();
    expect(hasObjectSchemaProperties(managerSteerParameters)).toBe(true);
    if (!hasObjectSchemaProperties(managerSteerParameters)) {
      throw new Error("manager_steer parameters should be an object schema with properties");
    }
    expect(managerSteerParameters).toMatchObject({
      properties: expect.objectContaining({
        gitBaseRef: expect.objectContaining({ type: "string", optional: true }),
      }),
    });
    expect(Object.keys(managerSteerParameters.properties)).toEqual(
      expect.arrayContaining(["gitBaseRef", "ref", "reviewDecision", "text", "timeoutMs", "wait", "workerId"]),
    );
    expect(managerSteer?.description).toContain("git base ref");
    expect(managerSteer?.description?.toLowerCase()).not.toContain("target ref");
  });
  ...
  it("starts a manager with its own linked Ralph run and schedules background execution", async () => {
    ...
    try {
      ...
      expect(manager.state.gitBaseRef).toBe("origin/main");
  ...
  it("records operator-facing messages and worker outcomes", async () => {
    ...
    try {
      ...
      await steer?.execute(
        ...
        {
          ref: manager.state.managerId,
          text: "Please summarize after merge",
          workerId: "record-manager-t-0001",
          reviewDecision: "approved",
          gitBaseRef: "origin/release",
        },
        undefined,
        undefined,
        createCtx(cwd),
          ...
          workerUpdates: [
            {
              workerId: "record-manager-t-0001",
              status: "completed",
              summary: "Merged into git base ref",
            },
          ],
        },
        undefined,
      ...
      expect(reread.state.gitBaseRef).toBe("origin/release");
      expect(
