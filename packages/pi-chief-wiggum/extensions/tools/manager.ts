...
import { buildParentSessionRuntimeEnv } from "@pi-loom/pi-ralph-wiggum/extensions/domain/runtime.js";
...
export function registerManagerTools(pi: ExtensionAPI): void {
  pi.registerTool({
    ...
    label: "manager_list",
    description:
      "List durable managers for Ralph-backed chief orchestration. Start broad with text when rediscovering a manager by title, git base ref, or latest summary; use status only when you intentionally want a narrow slice.",
    promptSnippet:
  ...
  pi.registerTool({
    ...
    label: "manager_start",
    description:
      "Create a durable manager from a spec, initiative, plan, ticket set, or free-text objective, optionally set `gitBaseRef` as the git base ref future workers should branch from, create its linked Ralph loop, schedule async-job-backed background execution, and optionally wait for the first update.",
    promptSnippet:
      "Start managers from whatever broad context you actually have. The manager loop can create missing research/spec/plan/ticket structure before it reconciles workers on the async-job-backed scheduler.",
    parameters: Type.Object({
      title: Type.String(),
      objective: Type.Optional(Type.String()),
      summary: Type.Optional(Type.String()),
      gitBaseRef: Type.Optional(Type.String()),
  ...
  pi.registerTool({
    ...
    label: "manager_wait",
    description:
      "Block until a manager durably advances, has something to say, reaches a terminal state, or the wait timeout expires. Use this after starting or steering a manager.",
    promptSnippet:
      "Use this to wait on the async-job-backed manager scheduler instead of polling manager_read manually.",
    parameters: Type.Object({
      ref: Type.String(),
      timeoutMs: Type.Optional(Type.Number()),
    }),
  ...
  pi.registerTool({
    ...
    label: "manager_steer",
    description:
      "Provide operator steerability between manager passes: answer escalations, provide guidance, record review decisions, or change the git base ref. Async-job-backed background execution is rescheduled automatically afterward, and you may optionally wait for the next update.",
    promptSnippet:
      "Use this between manager updates to answer what the manager asked, then let the async-job-backed scheduler continue on its own.",
    parameters: Type.Object({
      ref: Type.String(),
      text: Type.Optional(Type.String()),
...
export function registerInternalManagerTools(pi: ExtensionAPI): void {
  pi.registerTool({
    ...
    description:
      "Ensure ticket-bound workers and worktrees exist for the manager’s current ticket set, then start any queued worker Ralph loops in the background async job substrate. This is for the internal manager loop, not the normal operator-facing path.",
    parameters: Type.Object({ ref: Type.String() }),
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
