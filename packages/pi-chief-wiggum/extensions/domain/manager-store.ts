...
import type { RalphReadResult } from "@pi-loom/pi-ralph-wiggum/extensions/domain/models.js";
...
import { managerMessageRequiresOperatorInput } from "./models.js";
...
function defaultGitBaseRef(cwd: string): string {
  try {
...
function normalizeManagerState(state: ManagerState): ManagerState {
  return {
    ...
    gitBaseRef: normalizeOptionalString(state.gitBaseRef) ?? "HEAD",
...
function pendingBlockingOutgoingMessages(manager: ManagerReadResult): ManagerMessageRecord[] {
  return manager.messages.filter((message) => managerMessageRequiresOperatorInput(message));
}
...
function validateRecordableWorker(cwd: string, manager: ManagerReadResult, workerId: string): WorkerReadResult {
  const workerStore = createWorkerStore(cwd);
  let worker: WorkerReadResult;
  try {
    worker = workerStore.readWorker(workerId);
  } catch {
    throw new Error(`Unknown worker: ${workerId}`);
  }

  if (worker.state.managerId !== manager.state.managerId) {
    throw new Error(
      `Worker ${workerId} belongs to manager ${worker.state.managerId}, not ${manager.state.managerId}.`,
    );
  }

  if (["completed", "failed", "retired"].includes(worker.state.status)) {
    throw new Error(`Worker ${workerId} is already terminal with status ${worker.state.status}.`);
  }
...
export class ManagerStore {
    ...
    try {
      const { entity } = await upsertEntityByDisplayIdWithLifecycleEvents(
        storage,
        {
          ...
          tags: [manager.state.status, manager.state.gitBaseRef, manager.state.ralphRunId, ...manager.state.linkedRefs.ticketIds],
  ...
  listManagers(filter: ManagerListFilter = {}): ManagerSummary[] {
    ...
    return rows
      ...
      .filter((summary) => {
        ...
        return [summary.id, summary.title, summary.summary, summary.gitBaseRef].join(" ").toLowerCase().includes(lowered);
      })
  ...
  async createManagerAsync(input: CreateManagerInput): Promise<ManagerReadResult> {
    ...
    const baseState = normalizeManagerState({
      ...
      createdAt: timestamp,
      updatedAt: timestamp,
      gitBaseRef: normalizeOptionalString(input.gitBaseRef) ?? defaultGitBaseRef(this.cwd),
  ...
  async steerManagerAsync(ref: string, input: ManagerSteerInput): Promise<ManagerReadResult> {
    ...
    if (input.gitBaseRef?.trim()) {
      manager.state.gitBaseRef = input.gitBaseRef.trim();
    }
    if (input.text?.trim()) {
  ...
  private async ensureWorkers(manager: ManagerReadResult): Promise<string[]> {
    ...
    for (const ticketId of manager.state.linkedRefs.ticketIds) {
      ...
      const created = await workerStore.createWorkerAsync({
        ...
        workspace: { branch: expectedWorkerId, baseRef: manager.state.gitBaseRef },
  ...
  async recordManagerStepAsync(ref: string, input: ManagerCheckpointInput): Promise<ManagerReadResult> {
    ...
    await storage.transact(async (tx) => {
      for (const update of workerUpdates) {
        validateRecordableWorker(this.cwd, manager, update.workerId);
      }

      for (const update of workerUpdates) {
        await workerStore.recordWorkerOutcomeWithStorage(tx, identity, update.workerId, {
      ...
      } else if (pendingBlockingOutgoingMessages(manager).length > 0) {
