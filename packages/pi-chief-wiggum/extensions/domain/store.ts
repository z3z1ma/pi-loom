...
import type { LoomCanonicalStorage, LoomEntityRecord, LoomRuntimeAttachment } from "@pi-loom/pi-storage/storage/contract.js";
...
async function appendWorkerJournal(
  ...
  storageContext?: {
    ...
    identity: ReturnType<typeof openWorkspaceStorageSync>["identity"];
  },
): Promise<void> {
  const ticketStore = createTicketStore(cwd);
  if (storageContext) {
    await ticketStore.addJournalEntryWithStorage(storageContext.storage, storageContext.identity, ticketId, kind, text, metadata);
    return;
  }
  await ticketStore.addJournalEntryAsync(ticketId, kind, text, metadata);
}
...
export class WorkerStore {
  ...
  async recordWorkerOutcomeWithStorage(
    storage: LoomCanonicalStorage,
    identity: ReturnType<typeof openWorkspaceStorageSync>["identity"],
    ref: string,
    input: RecordWorkerOutcomeInput,
  ...
  async recordWorkerOutcomeAsync(ref: string, input: RecordWorkerOutcomeInput): Promise<WorkerReadResult> {
    ...
    return storage.transact((tx) => this.recordWorkerOutcomeWithStorage(tx, identity, ref, input));
  }
