...
export function isPendingManagerToOperatorMessage(
  message: Pick<ManagerMessageRecord, "direction" | "status">,
): boolean {
  return message.direction === "manager_to_operator" && message.status !== "resolved";
}

export function managerMessageRequiresOperatorInput(
  message: Pick<ManagerMessageRecord, "direction" | "kind" | "status">,
): boolean {
  return isPendingManagerToOperatorMessage(message) && message.kind !== "report";
}
