...
export function renderManagerList(managers: ManagerSummary[]): string {
  ...
  return managers
    .map(
      (manager) =>
        `${manager.id} [${manager.status}/${manager.managerRunStatus}] ${manager.title} git-base-ref=${manager.gitBaseRef} workers=${manager.workerCount} pending=${manager.pendingMessages}`,
    )
    .join("\n");
}
...
export function renderManagerDetail(result: ManagerReadResult): string {
  ...
  return [
    ...
    `Git base ref: ${result.state.gitBaseRef}`,
