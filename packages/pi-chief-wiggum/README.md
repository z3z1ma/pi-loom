# @pi-loom/pi-chief-wiggum

Pi Chief Wiggum is a thin orchestration layer on top of Pi Ralph Wiggum.

The package exists to add three things Pi Ralph Wiggum does not provide by itself:

- durable chief state around a manager objective
- managed git worktrees for ticket-bound worker branches
- an async-job-backed scheduler that advances raw Ralph loop iterations on the parent event loop

A manager is itself a Ralph loop. Workers are internal ticket-bound Ralph loops running in dedicated worktrees. Each bounded Ralph pass runs through a fresh session-runtime launch rather than CLI re-entry, and each manager or worker iteration is triggered through the shared async job substrate imported from Ralph Wiggum. Chief-to-worker communication remains durable and loop-native: the scheduler reacts to durable Ralph and chief state mutations rather than maintaining a hidden transcript-side control channel.

## AI-facing tool surface

`pi-chief-wiggum` exposes a manager-first tool surface:
...
- `manager_start` — create a manager from a spec, initiative, plan, ticket set, or free-text objective, optionally set `gitBaseRef` to the git ref workers should branch from, and schedule background execution on the current process
- `manager_wait` — block until the manager has something to say or finishes
- `manager_steer` — answer escalations, provide guidance, record review decisions, or update `gitBaseRef` when future workers should branch from a different git ref, then let background execution continue
...
4. The async-job-backed scheduler watches manager and worker durable state transitions while the parent process stays alive.
